        ...
        <header className="header">
            <div className="header__left">
                <div className="header__phone">
                    ...
                    <a href="tel:+996 705 188 955" className="header__phone-tel">+996 705 188 955</a>
                    <a href="tel:+996 555 188 955" className="header__phone-tel">+996 555 188 955</a>
                    <p className="header__phone-work"><span><svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M7.5 0C3.3587 0 0 3.3587 0 7.5C0 11.6413 3.3587 15 7.5 15C11.6413 15 15 11.6413 15 7.5C15 3.3587 11.6413 0 7.5 0ZM7.5 14.3478C3.71739 14.3478 0.652174 11.2826 0.652174 7.5C0.652174 3.71739 3.71739 0.652174 7.5 0.652174C11.2826 0.652174 14.3478 3.71739 14.3478 7.5C14.3478 11.2826 11.2826 14.3478 7.5 14.3478Z" fill="#FF9846"/>
</svg></span>работаем с 10:00 до 00:00</p>
                </div>
                <div className="header__address">
                    <p className="header__address-info">Город:</p>
            ...
            <div className="header__right">
                <ul className="header__list">
                    ...
                    <li>
                        <NavLink className="header__link" to="/delivery">Доставка и оплата</NavLink>
                    </li>
                </ul>
                <div className="header__search">
                    <input value={searchVal} type="search" className="header__search-input" style={{display : search ? 'block' : 'none'}} onChange={(e)=>
                    {searchHandler(e.target.value)
                    searchVal(e.target.value)
                    }}/>
                    <button type="button" className="header__search-btn" title="Пойск" onClick={()=> setSearch(!search)}>
                    ...
                    <ul className="header__search-list" style={{display : search && all.length ? 'flex' : 'none'}}>
                        {all.map((item,idx)=>(
                            <li key={idx} className="header__search-list-item">
                                <Link to={`/${item.categories}/product/${item.id}`}><img className="header__search-list-img" src={item.imageUrl} alt="" onClick={
                                    ()=> {
                                        setSearch(false)
                                         searchVal('')
                                        setAll([])

        ...
        <header className="header">
            <div className="header__left">
                <div className="header__phone">
                    ...
                    <p className="header__phone-work"><span><svg width="15" height="15" viewBox="0 0 15 15" fill="none"
...
<path
    ...
    fill="#FF9846"/>
</svg></span>работаем с 10:00 до 00:00
                    </p>
                </div>
            ...
            <div className="header__right">
                ...
                <div className="header__search">
                    <input value={searchVal} type="search" className="header__search-input"
                           style={{display: search ? 'block' : 'none'}} onChange={(e) => {
                        searchHandler(e.target.value)
                        setSearchVal(e.target.value)
                    }}/>
                    <button type="button" className="header__search-btn" title="Пойск" onClick={() => {
                        setSearch(!search)
                        setSearchVal('')
                        setAll([])
                    }
                ...
                <ul className="header__search-list" style={{display: search && all.length ? 'flex' : 'none'}}>
                    {all.map((item, idx) => (
                        <li key={idx} className="header__search-list-item">
                            <Link to={`/${item.categories}/product/${item.id}`}><img className="header__search-list-img"
